...
function Log($msg, $target=$null) {
    if (-not $target) { $target = $txtBuildLog }
    if ($target) {
...
$btnDevTest.Add_Click({
    ...
    try {
        ...
        # Kill any stale process on port 1420 to avoid "port already in use" error
        Log "Checking port 1420..."
        try {
            $netstat = netstat -ano | Select-String ":1420\s" | Select-String "LISTENING"
            foreach ($line in $netstat) {
                $pid = ($line -split '\s+')[-1]
                if ($pid -and $pid -ne "0") {
                    Log "Killing stale process on port 1420 (PID $pid)..."
