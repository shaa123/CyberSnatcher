    ...
    [void]$ps.AddScript({
        ...
        if (-not (Test-Path $WorkDir) -or -not $isGitRepo) {
            ...
            try {
                $proc = New-Object System.Diagnostics.Process
                $proc.StartInfo.FileName = "cmd.exe"
                $proc.StartInfo.Arguments = "/c cd /d `"$WorkDir`" && git fetch origin && git reset --hard origin/main 2>&1"
                $proc.StartInfo.UseShellExecute = $false
                $proc.StartInfo.RedirectStandardOutput = $true
                $proc.StartInfo.RedirectStandardError = $true
                $proc.StartInfo.CreateNoWindow = $true
        ...
        # ── Step 2: npm install ──
        BgStatus "running" "installing dependencies..."
        BgLog "> npm install" "#FFC8A0E8"

        try {
            $proc = New-Object System.Diagnostics.Process
            $proc.StartInfo.FileName = "cmd.exe"
            $proc.StartInfo.Arguments = "/c cd /d `"$WorkDir`" && npm install 2>&1"
            $proc.StartInfo.UseShellExecute = $false
            $proc.StartInfo.RedirectStandardOutput = $true
            $proc.StartInfo.RedirectStandardError = $true
            $proc.StartInfo.CreateNoWindow = $true
            ...
            while (-not $proc.StandardOutput.EndOfStream) {
                ...
                if ($line -match "error" -and $line -notmatch "0 error") { BgLog $line "#FFF08888" }
                elseif ($line -match "warn") { BgLog $line "#FFE8C878" }
                elseif ($line -match "added|packages") { BgLog $line "#FF88D8A8" }
                else { BgLog $line "#FFA898B8" }
            }
            $proc.WaitForExit()

            if ($proc.ExitCode -ne 0) {
                BgLog "npm install failed with exit code $($proc.ExitCode)" "#FFF08888"
                BgStatus "error" "npm install failed"
                $window.Dispatcher.Invoke([Action]{ $StartBtn.IsEnabled = $true; $FixCargoBtn.IsEnabled = $true; $DevOnlyBtn.IsEnabled = $true; $ProdBuildBtn.IsEnabled = $true })
                return
            }
            BgLog "Dependencies installed!" "#FF88D8A8"
        } catch {
            BgLog "ERROR: $_" "#FFF08888"
            BgStatus "error" "$_"
            $window.Dispatcher.Invoke([Action]{ $StartBtn.IsEnabled = $true; $FixCargoBtn.IsEnabled = $true; $DevOnlyBtn.IsEnabled = $true; $ProdBuildBtn.IsEnabled = $true })
        ...
        # ── Step 3: npm run tauri dev (detached) ──
