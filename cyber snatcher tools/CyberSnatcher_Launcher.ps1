...
<Window
    ...
    <Grid>
        ...
        <!-- ═══ SPLASH SCREEN (overlay, fades out) ═══ -->
        <Grid x:Name="SplashGrid" Panel.ZIndex="100" Background="#FF13101C">
            <StackPanel VerticalAlignment="Center" HorizontalAlignment="Center">

                <!-- Animated star icon -->
                <TextBlock Text="&#x2726;" FontSize="56" Foreground="#FFC8A0E8"
                           HorizontalAlignment="Center" Margin="0,0,0,12">
                    <TextBlock.RenderTransform>
                        <RotateTransform x:Name="SplashStarRotate" CenterX="0" CenterY="0" Angle="0"/>
                    </TextBlock.RenderTransform>
                </TextBlock>

                <TextBlock Text="CyberSnatcher" FontSize="34" FontWeight="Bold"
                           Foreground="#FFF0A0C0" HorizontalAlignment="Center"
                           Margin="0,0,0,4">
                    <TextBlock.Effect>
                        <DropShadowEffect Color="#FFF0A0C0" BlurRadius="20"
                                          ShadowDepth="0" Opacity="0.4"/>
                    </TextBlock.Effect>
                </TextBlock>

                <TextBlock Text="D E V   L A U N C H E R" FontSize="12" Foreground="#FF786888"
                           HorizontalAlignment="Center"
                           Margin="0,0,0,24"/>

                <!-- Static loading dots -->
                <StackPanel Orientation="Horizontal" HorizontalAlignment="Center">
                    <Ellipse Width="9" Height="9" Fill="#FFC8A0E8" Margin="0,0,8,0" Opacity="0.8"/>
                    <Ellipse Width="9" Height="9" Fill="#FFF0A0C0" Margin="0,0,8,0" Opacity="0.8"/>
                    <Ellipse Width="9" Height="9" Fill="#FFA0B8E8" Opacity="0.8"/>
                </StackPanel>
            </StackPanel>

            <!-- Splash fade-out (triggered from code) -->
        </Grid>

        <!-- ═══ MAIN CONTENT ═══ -->
        <Grid x:Name="MainGrid" Margin="24,20,24,18" Opacity="0">
            <Grid.RowDefinitions>
...
# ─── GET NAMED ELEMENTS ─────────────────────────────────
$SplashGrid   = $window.FindName("SplashGrid")
...
# ─── SPLASH DISMISS + MAIN FADE-IN ─────────────────────
$splashTimer = New-Object System.Windows.Threading.DispatcherTimer
$splashTimer.Interval = [TimeSpan]::FromMilliseconds(2200)
$splashTimer.Add_Tick({
    $splashTimer.Stop()

    # Fade out splash
    Start-FadeAnimation $SplashGrid 1 0 0.8

    # After splash fades, hide it and show main
    $showMain = New-Object System.Windows.Threading.DispatcherTimer
    $showMain.Interval = [TimeSpan]::FromMilliseconds(850)
    $showMain.Add_Tick({
        $showMain.Stop()
        $SplashGrid.Visibility = "Collapsed"
        Start-FadeAnimation $MainGrid 0 1 0.6
    })
    $showMain.Start()
})
